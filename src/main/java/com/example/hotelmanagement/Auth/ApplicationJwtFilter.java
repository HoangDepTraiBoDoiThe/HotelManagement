...
public class ApplicationJwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (!token.isBlank() && !utils.isTokenExpired(token)) {
                AuthUserDetail authUserDetail = (AuthUserDetail) authUserDetailService.loadUserByUsername(utils.getClaimProperty(token, Claims::getSubject));
